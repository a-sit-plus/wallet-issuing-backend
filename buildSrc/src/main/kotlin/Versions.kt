object VersionsBackend {
    const val ida = "3.9.0-SNAPSHOT"
    const val eupid = "2.2.0-SNAPSHOT"
    const val mdl = "1.1.0-SNAPSHOT"
    const val por = "1.1.0-SNAPSHOT"
    const val cor = "2.1.0-SNAPSHOT"
    const val eprescription = "1.1.0-SNAPSHOT"
    const val zxing = "3.5.2"
    const val `google-cbor` = "0.01.02"
    const val pgsql = "42.5.0"
    const val mockito = "5.1.0"
    const val okhttp = "4.11.0"
    const val scrimage = "4.0.39"
    const val vclib = "5.0.0-SNAPSHOT"
    const val attestation = "0.5.2"
    const val nimbus = "9.37.1"

    object spring {
        const val boot = "3.2.10"
        const val `dependency-management` = "1.1.6"
        const val `admin-starter-client` = "3.3.3"
        const val doc = "1.8.0"
    }

    object webjars {
        const val locator = "0.47"
        const val bootstrap = "5.3.1"
        const val jquery = "3.7.1"
        const val datatables = "1.13.5"
    }
}

object VersionsBackend {
    ...
    object spring {
        const val boot = "2.7.2"
        const val `dependency-management` = "1.0.12.RELEASE"
        const val `cloud-config-client` = "3.1.4"
        const val `admin-starter-client` = "2.7.4"
        const val doc = "1.6.11"
    }

    object serialization {
        const val json = "1.4.0"
    }

    object webjars {
        const val locator = "0.45"
        const val bootstrap = "5.2.0"
        const val jquery = "3.6.1"
        const val datatables = "1.12.1"
    }
}

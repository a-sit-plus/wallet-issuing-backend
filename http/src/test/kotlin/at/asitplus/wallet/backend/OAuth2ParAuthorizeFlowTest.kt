...
class OAuth2ParAuthorizeFlowTest {
    ...
    fun `par followed by authorize with request_uri succeeds when issuer_state is present`() = runTest {
        ...
        val parResult = mockMvc.post(Paths.ParUrl) {
        ...
        mockMvc.get(Paths.AuthorizeUrl) {
            ...
            status { isFound() }
            header { string(HttpHeaders.LOCATION, containsString("code=")) }
            header { string(HttpHeaders.LOCATION, containsString("state=$state")) }
        }
    }
}

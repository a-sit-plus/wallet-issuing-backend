...
class BackendConfiguration {
    ...
    fun issuerCredentialDataProvider(): IssuerCredentialDataProvider {
        return object : IssuerCredentialDataProvider {
            override fun getPupilIdCredentials(subjectId: String): List<Claim> {
                val defaultLifetime = configurationProperties.credentialLifetime
                val encodedPhoto = "/9j/4AAQSkZJRgABAQEBLAEsAAD//gATQ3JlYXRlZCB3aXRoIEdJTVD/4gIwSUNDX1BST0ZJTEUA\n" +
                ...
                return listOf(
                    Claim("firstname", "Susanne", "application/text", defaultLifetime),
                    Claim("lastname", "Meier", "application/text", defaultLifetime),
                    Claim("dateOfBirth", "1997-12-26", "application/text", defaultLifetime),
                    Claim("kennzahl", "00200000/00000004", "application/text", defaultLifetime),
                    Claim("schulName", "Quarto Testschule", "application/text", defaultLifetime),
                    Claim("schulAdresse", "1140 Wien, Breitenseer Straße 13", "application/text", defaultLifetime),
                    Claim("klasse", "3B", "application/text", defaultLifetime),
                    Claim("validUntil", "2021-07-31", "application/text", defaultLifetime),
                    Claim("picture", encodedPhoto, "image/jpeg", defaultLifetime)
                )
            }
        }
    }

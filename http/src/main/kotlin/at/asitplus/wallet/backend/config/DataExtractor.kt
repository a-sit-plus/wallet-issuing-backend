...
fun SdJwtCredentialScheme.buildSdJwtClaims(
    ...
    claims = when (sdJwtType) {
        ...
        Ida15BindingClaims.VCT -> userInfo.buildIda15BindingClaims(false)
...
fun OidcUserInfoExtended.buildIda15BindingClaims(useSd: Boolean) =
    with(Ida15BindingClaims) {
        listOfNotNull(
            claim(SIGNER_CERTIFICATE, useSd) {
                getClaimAsString("urn:pvpgvat:oidc.eid_signer_certificate")
                    ?: """
            ...
            claim(IDENTITY_TYPE, useSd) {
                getClaimAsString("urn:pvpgvat:oidc.eid_identity_status_level")
                    ?: "http://eid.gv.at/eID/status/identity"
            },
            claim(ISSUING_COUNTRY, useSd) {
                getClaimAsString("urn:pvpgvat:oidc.eid_issuing_nation")
                    ?: "AT"
            },
            claim(EID_STATUS, useSd) {
                getClaimAsString("urn:pvpgvat:oidc.eid_ida_level")
                    ?: "urn:eidgvat:eid.status.full"
            },
            claim(VSZ_SHA256, useSd) {
                "vz8MLsylrDtSM6bScC9Hp9qgSLVBHrNTQMsnpapFL4s"
            },
        )
    }
